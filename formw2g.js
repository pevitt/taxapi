...
module.exports = function(server, db_config){
  ...
  server.post('/w2g/:userId', (req, res, next) => {
    if(req.params.userId && isInteger(req.params.userId) ){
          ...
          req.body.hasOwnProperty('ZipW') && req.body.ZipW  != "")
      {
          ...
          _sqlparams.push(req.body.AlteredW2);
          _sqlparams.push(req.body.CorrectedW2);
          _sqlparams.push(req.body.LotteryW);
          _sqlparams.push(req.body.ElectronicW);
          _sqlparams.push(req.body.CostTicket);
          _sqlparams.push(req.body.GrossWin);
  ...
  server.del('/w2g/:w2gId/:userId', (req, res, next) => {
    if(req.params.w2gId && isInteger(req.params.w2gId) && req.params.userId && isInteger(req.params.userId)){
      ...
      connection.query(queryInsert , [req.params.userId, req.params.w2gId], function (err, result, fields) {
        ...
        if(result != null && result[0] != null){
          ...
          connection.query(queryInsert , [req.params.userId, req.params.w2gId], function (err, result, fields) {
            ...
            res.send(200, {success: false, message:"W2G Form deleted successfully"});
            return next(false);
          });

        }else{
          //message unemployment form doesn't match the user
          res.send(200, {success: false, message:"W2G Form doens't match with the user"});
          return next(false);
        }
      ...
      res.send(200, {success: false, message: "The W2G id is required"});
      return next(false);  
    }
    return next();
  });
